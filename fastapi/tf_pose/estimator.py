...
model = 1  # 定义model变量 选择体测项目
count = 0  # 定义count变量 计数
pose_state = 2  # 定义pose_state变量 指示当前姿势状态    pose_state不能初始化为0或1 否则会对计数判断产生影响
out_angle = []  # 将所有身体角度
wrong_pose_flag = 0  # 判断姿态是否标准 不标准时置1
pullup_judge = 0  # 判断引体向上手臂角度是否到达设定值
...
class TfPoseEstimator:
    ...
    def draw_humans(npimg, humans, imgcopy=False):
        global count, pose_state, model, out_angle  # global使其成为全局变量
        ...
        for human in humans:
            ...
            for i in range(common.CocoPart.Background.value):
                if i not in human.body_parts.keys():  # 若未检测出i对应的点
                    continue
